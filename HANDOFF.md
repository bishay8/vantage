...
- `index.html` — loader: Tailwind Play CDN + React 18 UMD + Babel Standalone (in-browser JSX transpile). Has all SEO/OG/Twitter meta + favicon. Loads `vantage.jsx?v=N` — **bump N on every change to bust the browser cache.** Currently `?v=22`.
...
A **live interaction/state-bug audit** (6 module clusters, behavioral not static) then fixed **12 runtime bugs** the static passes missed — including a **CRITICAL** one: editing/deleting a Portfolio holding after sorting hit the *wrong* holding (silent data corruption). Root cause across several modules was the React `key={i}` / index-based-handler anti-pattern. Fixes: stable `id`s + functional setState + key-by-id in Portfolio, GoalPriority, CapBudget; `cc_` namespace on custom-category keys (no more double-count vs base fields like "food"); Bond YTM/Spread NaN guard; hiding the active module's tier now returns Home (no orphan); retirement projector clamps extreme rate/years; goal celebration only fires on added savings (not lowering the target); CustomizePanel/ReminderStub remount on reopen (no stale state). The critical Portfolio fix was verified live (edit-after-sort now targets exactly the right row).

**Latest session — dark mode + real legal copy + features, live at `?v=22`:**
- **Dark mode SHIPPED** (OS-driven `dark:` *media* variants — no config / no Golden-Rule #3 exception). Warm reading-room palette (`#15171c` bg / `#1c1f26` surface / `#eef1f6` text), existing accents kept. Every shell/primitive/module surface + all input fields themed (~1,270 `dark:` tokens). Verified 19/19 modules: 0 horizontal overflow, 0 unthemed light surfaces; light mode provably unchanged (variants are additive). The dead hidden boolean toggle was retired — a *manual* toggle would now need `darkMode:'class'` config (a Golden-Rule #3 call).
- **Legal copy replaced** with real **US-federal + California (CCPA/CPRA)** Terms (16§)/Privacy (17§)/Disclaimer (10§): not-an-RIA (Advisers Act 1940)/not-a-broker-dealer (Exchange Act 1934), binding arbitration + 30-day opt-out + small-claims carve-out, CCPA/CPRA rights, Shine the Light, COPPA, $100 liability cap. Drafted + adversarially reviewed. **Still needs a licensed attorney** before public/paid launch — the `LegalModal` dev comment lists the `attorneyFlags`.
- **Wave-3:** MarketLab indicators memoized (`useMemo`); candle legend now uses ▲/▼ shape, not color alone. **Dropped** the 18+ age-gate + granular consent (unneeded for an educational, no-money-movement tool — COPPA is <13; 18+ only matters once there's trading/money movement).
- **"Build your own" line items:** a 3rd CustomizePanel tab to hand-add asset/liability/expense items (no backend; `cc_`-namespaced, collision-safe).
- **REBRAND DECIDED → "Sentirel"** (launch on `sentirel.io`; repo/code stays `vantage.jsx` until a rename pass). "Vantage" is crowded in finance (VantageScore / Vantage Bank / Vantage Markets). After ~7 naming rounds — every real/dictionary/soft word was taken or collided (incl. a live $NUMA stablecoin, the Numba Python library, a Centra-Tech crypto scam) — the chosen coined name is **sentirel** = *sentinel + sentir (to sense)* = calm, watchful clarity; on-brief for plain-English honest finance + the "Grinch of Finance" persona. Trust neighborhood clean, low trademark in class 36/9/42, @sentirel handles free; only catch: exact `.com` is held by a domain flipper → launch `.io` or buy it. Clean backups: **grounel** (zero concessions) and **cernuma** (free `.com`). NOTE: the "Grinch of Finance" Substack/X persona is the launch CHANNEL + voice, **not** the app name (Dr. Seuss/WB "Grinch" trademark). **Owner's offline gate before committing repo/URL:** register `sentirel.io` + grab `@sentirel` everywhere, USPTO knockout in class 36/9/42, attorney clearance.
- **Heads-up:** a parallel audit process (commits authored "…found by behavioral/content/math audit") was committing+pushing to this same working tree during the session — coordinate to avoid shared-tree races.
...
- **Legal:** point-of-use "educational, not advice" disclaimers (`<AdviceNote>`) on Market Lab, Options, Tax Estimator, Tax Optimizer; Terms/Privacy/Disclaimer modals (now real US/CA-grounded copy as of the latest session — attorney review still pending); accurate BYOK privacy wording.
...
1. **Backend — NOW THE ACTIVE WORK (Phase 0 delivered).** Full blueprint in **`BACKEND.md`** (stack: Supabase — Postgres + Auth + RLS + Edge Functions; schema, the precise `// FUTURE:` swap map, phased plan, cost, compliance). Phase-0 artifacts are committed + deploy-ready in **`backend/`**: `migrations/0001_init.sql` (schema + RLS, users own their rows) and `functions/categories/index.ts` (the Anthropic proxy that retires BYOK — same prompt/model, key now server-held, JWT-gated, daily quota). **Next:** owner stands up a Supabase project (run the SQL, enable magic-link, deploy the function, set `ANTHROPIC_API_KEY`) → sends back Project URL + anon key → then wire the frontend (add `supabase-js` as a CDN global like React, magic-link sign-in + **guest mode**, then the swaps) and verify live. ⚠ **Update the Privacy Policy before the first real row is stored** — the shipped "we collect nothing today" copy becomes false. (SOC 2 itself is a later enterprise-trust cert, **not** a launch gate.) When wiring, replace these (`// FUTURE:`):
...
2. ~~**Full dark mode**~~ **DONE this session** (OS-driven `dark:` media variants). Optional follow-ups: a *manual* light/dark toggle (needs inline `darkMode:'class'` config — a Golden-Rule #3 judgment call), and theming the brief light boot-screen flash in `index.html`'s `<style>` block.
3. ~~**Replace TEMPLATE legal copy**~~ **DONE this session** — real US-federal + California (CCPA/CPRA)-grounded copy. Remaining: a **licensed attorney** must review before any public/paid launch (see the `attorneyFlags` in the `LegalModal` dev comment — arbitration enforceability, the $100 cap under Civ. Code 1668, LLC/entity formation, RIA/securities status, trademark, CCPA thresholds, clickwrap acceptance).
...
6. **Wave-3** — ~~memoize MarketLab indicators~~ **DONE**; ~~color-only signal labels~~ **DONE** (candle legend ▲/▼). Dropped the 18+ age-gate + granular consent (unneeded for educational/no-money-movement scope; revisit only if trading/money movement is added). New nice-to-have: let "Build your own"/manual line items *augment* an existing AI/template set (today it builds a fresh set).
